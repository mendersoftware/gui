...
test.describe('Files', () => {
  ...
  test('allows artifact downloads', async ({ demoArtifactVersion, loggedInPage: page }) => {
    ...
    exec(`mender-artifact read --no-progress ${downloadTargetPath}`, (err, stdout, stderr) => {
      ...
      const versionInfo = artifactName.substring(artifactName.indexOf(expectedArtifactName), expectedArtifactName.length + 2);
      expect(versionInfo).toEqual(demoArtifactVersion.artifactVersion);
      const { 'data-partition.mender-demo-artifact.version': updateVersion } = artifactInfo.Updates.Provides;
