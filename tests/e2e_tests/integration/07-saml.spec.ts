...
test.describe('SAML Login', () => {
  ...
  test.describe('SAML login via sso/id/login', () => {
    test.afterAll(async ({baseUrl, samlCredentials}, testInfo) => {
      console.log(`Finished ${testInfo.title} with status ${testInfo.status}. Cleaning up.`);
      const response = await axios({
        url: `${baseUrl}api/management/v1/useradm/users?email=${encodeURIComponent(samlSettings['credentials'][samlCredentials]['email'])}`,
        method: 'GET',
        headers: {
      ...
      if (responseStatus != 200 || response.data.length < 1) {
        console.log(`${samlSettings['credentials'][samlCredentials]['email']} does not exist.`);
      } else {
        ...
        expect(response.status).toBe(200);
        console.log(`removed user ${samlSettings['credentials'][samlCredentials]['email']}/${userId}.`);
      }
    });

    // Setups the SAML/SSO login with samltest.id Identity Provider
    test('Set up SAML', async ({environment, context, baseUrl, page, password, username}) => {
      ...
      // Click input[name="userfile"]
      await page.locator('input[name="userfile"]').click();
      // Upload m
      await page.locator('input[name="userfile"]').setInputFiles('fixtures/service_provider_metadata.xml');
    ...
    test('Creates a user without a password', async ({ environment, samlCredentials, context, baseUrl, page }) => {
      test.skip(environment !== 'staging');
      const response = await axios({
        url: `${baseUrl}api/management/v1/useradm/users?email=${encodeURIComponent(samlSettings['credentials'][samlCredentials]['email'])}`,
        method: 'GET',
      ...
      if (responseStatus != 200 || response.data.length < 1) {
        ...
        await page.locator('[placeholder="Email"]').fill(samlSettings['credentials'][samlCredentials]['email']);
        // Click text=Create user
        await page.locator('text=Create user').click();
        await page.waitForTimeout(timeoutOneSecond);
        await expect(page.locator(`text=${samlSettings['credentials'][samlCredentials]['email']}`)).toHaveText(samlSettings['credentials'][samlCredentials]['email']);
        console.log(`${samlSettings['credentials'][samlCredentials]['email']} created.`);
      } else {
        console.log(`${samlSettings['credentials'][samlCredentials]['email']} already exists.`);
      }
    });

    // This test calls auth/sso/${id}/login, where id is the id of the identity provider
    // and verifies that login is successful.
    test('User can login via sso/login endpoint', async ({environment, samlCredentials, baseUrl, browser}) => {
      test.skip(environment !== 'staging');
      console.log(`logging in via ${baseUrl}api/management/v1/useradm/auth/sso/${metadataId}/login (credentials set:${samlCredentials})`);
      ...
      await page.locator('input[name="j_username"]').fill(samlSettings['credentials'][samlCredentials]['login']);

      // Press Tab
      await page.locator('input[name="j_username"]').press('Tab');

      // Click input[name="j_password"]
      await page.locator('input[name="j_password"]').click();

      // Fill input[name="j_password"]
      await page.locator('input[name="j_password"]').fill(samlSettings['credentials'][samlCredentials]['password']);
      ...
      await page.waitForSelector(`text=${samlSettings['credentials'][samlCredentials]['email']}`);
    });
  });
});
