...
export const PhysicalDeviceOnboarding = ({
  ...
  const onSelect = (_, deviceType, reason) => {
  ...
  const deviceTypeHelperText = selection.includes(' ') ? 'Device types can not contain spaces' : null;
  const steps = {
    1: (
          ...
          <Autocomplete
            ...
            renderInput={params => (
              <TextField
                ...
                style={{ marginTop: 0 }}
                FormHelperTextProps={{ classes: { root: 'warning' } }}
                helperText={deviceTypeHelperText}
              />
            )}
            value={selection}
          />
