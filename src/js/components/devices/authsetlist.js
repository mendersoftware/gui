...
var AuthsetList = createReactClass({
	...
	render: function() {
        ...
        expanded = <div className="expand-confirm">
			...
			var actionButtons = expanded ? "Confirm " + self.state.newStatus +"?" : (
				<div className="actionButtons">
					{((authset.status !== "accepted") && (authset.status !== "preauthorized") && !self.props.limitMaxed) ? <a onClick={self.props.total>1 ? self.setConfirmStatus.bind(null, authset, "accept", index) : self.confirm.bind(null, authset, "accept") }>Accept</a> : <span className="bold muted">Accept</span> }
					...
					<a onClick={(self.props.total>1 || self.props.device.status!=="pending") ? self.setConfirmStatus.bind(null, authset, "dismiss", index) : self.confirm.bind(null, authset, "dismiss")}>Dismiss</a>
				</div>
			);
        ...
        <TableRow hoverable={true} style={self.props.active ? {"backgroundColor": "#e9f4f3"} : {}} className={expanded ? "expand" : null} key={index}>
          ...
          <TableRowColumn>
          	{self.props.loading === authset.id ? <span>Updating status <Loader table={true} waiting={true} show={true} style={{height:"4px", marginLeft:"10px"}} /></span> : actionButtons}
          </TableRowColumn>
