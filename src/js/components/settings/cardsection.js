...
const CheckoutForm = (props) => {
  ...
  const handleSubmit = async (event) => {
    ...
    props.startUpgrade(props.org.id)
      .catch(err => {
        props.setSnackbar(preformatWithRequestID(err.res, err.res.body.error), null, 'Copy to clipboard'); 
        setLoading(false);
      })
      .then(data => {
        confirmCard(data.body.secret);
      });
  }
  ...
  const confirmCard = async (secret) => {
    ...
    try {
      ...
      if (result.error) {
        ...
        // to our backend
        props.completeUpgrade(props.org.id, props.plan)
          .catch((err) => {
            setLoading(false);
