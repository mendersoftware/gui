...
const reduceReceivedDevices = (devices, ids, state, status) =>
  devices.reduce(
    (accu, device) => {
      ...
      device.inv_check_in_time = system['check_in_time'] ?? stateDevice.inv_check_in_time;
      device.devauth_check_in_time = device.check_in_time ?? stateDevice.devauth_check_in_time;
      ...
      device.isOffline = new Date(device.inv_check_in_time) < new Date(state.app.offlineThreshold) || device.inv_check_in_time === undefined;
...
const deriveInactiveDevices = deviceIds => (dispatch, getState) => {
  ...
  const devices = deviceIds.reduce(
    (accu, id) => {
      ...
      if (device && device.inv_check_in_time > yesterdaysIsoString) {
        accu.active.push(id);
      } else {
