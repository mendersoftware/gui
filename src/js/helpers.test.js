...
describe('getDebConfigurationCode function', () => {
  let code;
  beforeEach(() => {
    code = getDebConfigurationCode('192.168.7.41', false, false, null, 'raspberrypi3', false, undefined);
  });
...
`
  ...
  describe('configuring devices for hosted mender', () => {
    ...
    it('should contain sane information for hosted calls', async () => {
      code = getDebConfigurationCode(undefined, true, false, 'token', 'raspberrypi3', false, undefined);
      expect(code).toMatch(
        `wget -q -O- https://get.mender.io | sudo bash -s -- --demo --commercial --auth hosted --jwt-token "eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9..qVgYdCzLTf8OdK9uUctqqaY_HWkIiwpekuGvuGQAXCEgOv4bRNDlZRN_ZRSbxQoARG3pquhScbQrjBV9tcF4irTUPlTn3yrsXNO17DpcbTVeKRkb88RDtIKiRw3orVZ_GlIb-ckTQ5dS-Nqlyyf3Fmrhca-gwt6m_xv2UrmJK6eYYTMfggdRRWb-4u7mEkBI_pHPMTQrT8kJ2BeX-vHgazH9AoH0k85LHtFZQXD7pXHlDZRnLxJXukncwMGDmF17374gavYAIyDIzcC8sEBMDnVXgpikeA1sauzirqix6mAVs6XmxdQO7aF0wfXO1_PTYUA3Nk1oQfMYNlEI3U9uLRJRZIq2L8fmrrBryhstKd4y0KlBbGAQrx8NtRkgajjd1ljMfPBUEZrb7uSerVjneiO-aIBO76CuH0zdklphIjpGJeogkBhe8pAYNggp1XsZHgpZfl7IE5faKaDkMGnutaea--Czor6bhqUNCuY4tR0cpQJbNwy6LS9o1CFy4Log" && \\
sudo bash -c 'DEVICE_TYPE="raspberrypi3" && \\
TENANT_TOKEN="token" && \\
...
`
    ...
    it('should contain sane information for staging preview calls', async () => {
      code = getDebConfigurationCode(undefined, true, false, 'token', 'raspberrypi3', true, undefined);
      expect(code).toMatch(
        `wget -q -O- https://get.mender.io/staging | sudo bash -s -- --demo -c experimental --commercial --auth hosted --jwt-token "eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9..qVgYdCzLTf8OdK9uUctqqaY_HWkIiwpekuGvuGQAXCEgOv4bRNDlZRN_ZRSbxQoARG3pquhScbQrjBV9tcF4irTUPlTn3yrsXNO17DpcbTVeKRkb88RDtIKiRw3orVZ_GlIb-ckTQ5dS-Nqlyyf3Fmrhca-gwt6m_xv2UrmJK6eYYTMfggdRRWb-4u7mEkBI_pHPMTQrT8kJ2BeX-vHgazH9AoH0k85LHtFZQXD7pXHlDZRnLxJXukncwMGDmF17374gavYAIyDIzcC8sEBMDnVXgpikeA1sauzirqix6mAVs6XmxdQO7aF0wfXO1_PTYUA3Nk1oQfMYNlEI3U9uLRJRZIq2L8fmrrBryhstKd4y0KlBbGAQrx8NtRkgajjd1ljMfPBUEZrb7uSerVjneiO-aIBO76CuH0zdklphIjpGJeogkBhe8pAYNggp1XsZHgpZfl7IE5faKaDkMGnutaea--Czor6bhqUNCuY4tR0cpQJbNwy6LS9o1CFy4Log" && \\
sudo bash -c 'DEVICE_TYPE="raspberrypi3" && \\
TENANT_TOKEN="token" && \\
echo "Running mender setup for staging.hosted.mender.io" && \\
mender setup \\
...
`
  ...
  describe('configuring devices for fancy.enterprise.on.prem', () => {
    ...
    it('should contain sane information for enterprise on-prem calls', async () => {
      code = getDebConfigurationCode(undefined, false, true, 'token', 'raspberrypi3', false, 'username');
      expect(code).toMatch(
        `wget -q -O- https://get.mender.io | sudo bash -s -- --demo --commercial --auth on-prem --username "username" && \\
sudo bash -c 'DEVICE_TYPE="raspberrypi3" && \\
TENANT_TOKEN="token" && \\
